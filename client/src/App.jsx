...
function App() {
  ...
  /* New Game Helpers */
  const [captionChoices, setCaptionChoices] = useState([-1, -1, -1]);

  const addToCaptionChoices = function (round, caption_index) {
    const newCaptionChoices = [...captionChoices];
    if (round >= 0 && round < newCaptionChoices.length) {
      newCaptionChoices[round] = caption_index;
      setCaptionChoices(newCaptionChoices);
    }
  }

  const resetChoices = function () {
    setCaptionChoices([-1, -1, -1]);
  }
    ...
    <LoggedInContext.Provider value={value_loggedInContext}>
      <div className="main-content">
        <Routes>
          ...
          <Route path="/play"
            element={<Play game={game} choices={captionChoices} addToCaptionChoices={addToCaptionChoices} resetChoices={resetChoices} roundMax={isLoggedIn ? 2 : 0}/>}
          />
